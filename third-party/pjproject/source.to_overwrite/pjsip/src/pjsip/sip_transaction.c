				        ...
				        pjsip_tx_data *tdata )
{
    ...
    /* if transaction state is destroyed, cancel this process. */
    if (tsx->state >= PJSIP_TSX_STATE_DESTROYED) {
        PJ_LOG(2,(tsx->obj_name, "[pjsip_tsx_send_msg] transaction state = %s, cancel this process.", state_str[tsx->state]));
